  ...
  | **PERF-2** | `PaletteGenerator` on main thread | ✅ **FIXED** | `mini_player.dart`, `now_playing_screen.dart`, `playlist_details_screen.dart` |
  | **PERF-3** | `AnimatedMeshGradient` continuous 60fps | ✅ **FIXED** | `now_playing_screen.dart` (replaced with canvas `_AnimatedBackground`) |
  | **PERF-4** | Heavy `BackdropFilter` sigma values | ✅ **FIXED** | `home_screen.dart` σ26→16, `search_screen.dart` σ16→10, `playlist_details_screen.dart` σ48→28 |
  | **PERF-5** | Unbounded stagger delays on list items | ✅ **FIXED** | `home_screen.dart`, `library_screen.dart`, `playlist_details_screen.dart` |
  | **PERF-6** | 5000 songs on main thread, no isolate | ✅ **FIXED** | `subsonic_service.dart` (`compute` in `_get`, `getAllSongs`), `library_provider.dart` (sort) |
  | **PERF-7** | `_loadPalette` async side-effect inside `build()` | ✅ **FIXED** | `mini_player.dart` (moved to `addPostFrameCallback`) |
