...
## Summary
- 1585 nodes · 2807 edges · 113 communities (104 shown, 9 thin omitted)
...
- [[_COMMUNITY_Album Art Shader Extensions|Album Art Shader Extensions]]
- [[_COMMUNITY_Unsafe Cast Standard|Unsafe Cast Standard]]
- [[_COMMUNITY_Theme Hardcoded Color Standard|Theme Hardcoded Color Standard]]
- [[_COMMUNITY_Android Gradle Build Config|Android Gradle Build Config]]
- [[_COMMUNITY_Linux Flutter App Header|Linux Flutter App Header]]
- [[_COMMUNITY_Linux Message Channel Header|Linux Message Channel Header]]
- [[_COMMUNITY_Linux Binary Codec Header|Linux Binary Codec Header]]
...
Cohesion: 0.09
Nodes (36): edit_playlist_screen.dart, dispose, initState, build, build, dispose, EditPlaylistScreen, _EditPlaylistScreenState (+28 more)
...
Cohesion: 0.08
Nodes (33): DownloadStateNotifier, OfflineService, _performDownload, progressOf, _set, _setFailed, statusOf, _buildContainer (+25 more)
...
Cohesion: 0.11
Nodes (27): ../../../core/theme.dart, build, Container, QueueScreen, _QueueTile, Slidable, AlbumCard, _AlbumPlaceholder (+19 more)
...
Cohesion: 0.12
Nodes (24): dart:convert, dart:math, LrcLibService, LyricsResult, _cache, LyricsRepository, _structuredLinesToLrc, ListeningLogService (+16 more)
...
Cohesion: 0.13
Nodes (21): build, Container, dispose, Function, ListTile, _onSearchChanged, Padding, _SearchHeaderDelegate (+13 more)
...
Cohesion: 0.15
Nodes (16): ../database/app_database.dart, PlaylistCache, SearchHistory, _cacheSongs, compute, PlaylistController, compute, CachedPlaylistResult (+8 more)
...
Cohesion: 0.12
Nodes (16): GestureDetector, SliverToBoxAdapter, SliverPadding, build, GestureDetector, initState, _NewReleaseCard, NewReleasesScreen (+8 more)
...
Cohesion: 0.12
Nodes (12): HealthResponse, _parseInt, _parseInt, ProfileResponse, _parseInt, SessionStatusResponse, AlbumStat, ArtistStat (+4 more)
...
Cohesion: 0.15
Nodes (13): code:dart (// ❌ NEVER — non-reactive, stale data), code:dart (// ✅ REQUIRED — reactive), code:dart (// ❌ NEVER), code:dart (// Required tests:), code:dart (// Required tests:), code:dart (// Required tests:), code:dart (// Required tests:), code:dart (// Required tests:) (+5 more)
...
Cohesion: 0.17
Nodes (11): code:block1 (BUILD → SELF-AUDIT → TEST), code:block47 (STEP 1: READ), code:block48 (## Task Audit: [Task Name]), Flutter Production Build & Audit Skill, Full Task Execution Flow, Phase 2: SELF-AUDIT — Mandatory After Every Task, Phase 3: TEST — Mandatory Test Plan Per Task, Post-Task Audit Report Format (+3 more)
...
Cohesion: 0.18
Nodes (10): dart:async, _fakeSong, ListeningLogService, main, _makeService, MockHttpClient, _stubPost, _stubPostTimeout (+2 more)
...
### Community 85 - "Album Art Shader Extensions"
Cohesion: 0.67
Nodes (3): code:dart (// ❌ NEVER in new files), code:dart (// ✅ REQUIRED), No StateNotifier in New Code (Riverpod 3.0+)
...
- **Why does `package:flutter_riverpod/flutter_riverpod.dart` connect `Settings Screen` to `Now Playing & Queue UI`, `Replay & New Releases Screens`, `Library & Favorites Screens`, `Made For You Detail Screen`, `Playlist Detail Screen`, `Song Picker Screen`, `Made For You Widgets`, `Player Provider & Controls`, `Library & Playlist Providers`, `Settings Provider`, `Download Provider & State`, `Listening Stats Provider`, `Replay Provider`, `Audio Handler & Core Logic`, `Recommendation Service`, `Listening Event Collector`, `Scrobble Service`, `BPM Analyzer Service`, `Cache Settings Service`, `Replay Gain Service`, `Transcoding Service`, `Replay Upload Service`, `Listening Log Service`, `AI Shuffle Features`, `UI Dialogs`, `Core Themes & Transitions`, `Linux Entry Point`, `AI Shuffle UI Components`, `Core Exception Classes`, `App Database Schema`, `Navigation Animations`, `AI Shuffle Repositories`, `Palette Cache Logic`, `Analytics Tables`, `Player Notifier Bug Analysis`, `Replay Gain Service`, `Test Plan & Initialization`?**
  _High betweenness centrality (0.104) - this node is a cross-community bridge._
- **Why does `package:flutter/material.dart` connect `Made For You Detail Screen` to `Now Playing & Queue UI`, `Playlist Detail Screen`, `Settings Screen`, `Made For You Widgets`, `Player Provider & Controls`, `Library & Playlist Providers`, `Settings Provider`, `Listening Stats Provider`, `Audio Handler & Core Logic`, `Recommendation Service`, `Scrobble Service`, `Cache Settings Service`, `Replay Gain Service`, `Transcoding Service`, `Replay Upload Service`, `Listening Log Service`, `AI Shuffle Features`, `UI Dialogs`, `Core Constants & Exceptions`, `Hive Database & Migration`, `Core Themes & Transitions`, `Android Entry Point`, `App Database Schema`, `Navigation Animations`, `AI Shuffle Repositories`, `Palette Cache Logic`, `Analytics Tables`, `LRC Parser & Lyrics UI`, `Knowledge Graph Rules`, `Shuffle Logic & Settings`, `Player Notifier Bug Analysis`, `Testing Requirements`, `Lyrics Background Widget`, `Test Plan & Initialization`, `Build Standards`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Why does `../../models/song.dart` connect `BPM Analyzer Service` to `Now Playing & Queue UI`, `Replay & New Releases Screens`, `Made For You Detail Screen`, `Playlist Detail Screen`, `Song Picker Screen`, `Made For You Widgets`, `Playlist Header Widgets`, `Player Provider & Controls`, `Library & Playlist Providers`, `Download Provider & State`, `Replay Provider`, `Audio Handler & Core Logic`, `Playlist Cache Service`, `Scrobble Service`, `Search History Service`, `Replay Gain Service`, `Transcoding Service`, `Listening Log Service`, `AI Shuffle Features`, `Device Utilities`, `Linux Entry Point`, `App Database Schema`, `UI Component Rationales`, `No Lyrics Card`, `Testing Requirements`?**
