...
chunks = []
for i in range(1, 10):
    path = Path(f'graphify-out/.graphify_chunk_{i:02d}_files.txt')
    if path.exists():
...
/home/subi/Documents/flutter-_navi/graphify-out/.graphify_chunk_{chunk_num:02d}.json
"""

for i, chunk in enumerate(chunks, 1):
    prompt = prompt_template.format(chunk_num=i, total_chunks=len(chunks), file_list=chunk)
    Path(f'graphify-out/.prompt_{i:02d}.txt').write_text(prompt)
