...
class RecommendationService extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    ...
    if (skipJson != null) {
      try {
        ...
        debugPrint('Error loading skip data: $e');
      }
    }
    ...
    if (timeJson != null) {
      try {
        ...
        debugPrint('Error loading time data: $e');
      }
    }
  ...
  Future<void> trackSongPlay(
    ...
    // Genre affinity — non-nullable but can be empty
    if (song.genre.isNotEmpty) {
      final gw = completed ? 1.2 : 0.6;
      _genreAffinity[song.genre] = (_genreAffinity[song.genre] ?? 0) + gw;
  ...
  double calculateSongScore(Song song, {int? currentHour}) {
    ...
    if (_timePatterns.containsKey(hour) && song.genre.isNotEmpty) {
